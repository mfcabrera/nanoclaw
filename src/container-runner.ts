...
function buildContainerArgs(
  ...
  // hostGatewayArgs() handles Linux; on macOS we need --add-host when
  // MCP gateways are in use so host.docker.internal resolves reliably.
  if (hasMcpGateways && hostGatewayArgs().length === 0) {
    // macOS Docker Desktop already has host.docker.internal, but this is harmless
  }
